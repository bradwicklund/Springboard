...
SELECT
	...
	m.joindate
FROM Members AS m
JOIN (SELECT MAX(joindate) AS max_joindate FROM Members) AS t
	ON m.joindate = t.max_joindate;
...
SELECT
    m1.surname,
    m1.firstname,
    CONCAT(m2.firstname, ' ', m2.surname) AS recommender_name
FROM Members AS m1
